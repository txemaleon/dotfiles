...
### Global Google Search Console and Bing CLI

The host has `search-console-mcp@2.1.3` installed globally and registered as a
Codex MCP server. Agents should prefer its CLI for deterministic, first-party
Google Search Console and Bing Webmaster Tools data:
...
Read [search-console-mcp.md](./search-console-mcp.md) for property resolution,
project scoping, read-only constraints and additional recipes. Do not guess a
property identifier or use write tools from an automated audit.
