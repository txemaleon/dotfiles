...
**Authoritative Search Console data:**

When the target has a configured Google Search Console or Bing Webmaster Tools
property, use the global `search-console-mcp` CLI for first-party search
performance, indexing and sitemap evidence. Read
[`references/search-console-mcp.md`](./references/search-console-mcp.md) before
querying it. Resolve the exact property identifier returned by `sites_list`,
limit every project-scoped task to that project's public domain, and keep
automated audits read-only.
...
```bash
# First resolve the exact property identifiers available to each engine.
search-console-mcp run sites_list --engine=all --format=json

# Then inspect the target URL using the exact property identifier returned.
...
- [references/search-console-mcp.md](./references/search-console-mcp.md) - Global Google/Bing CLI, project scoping and safe query recipes
